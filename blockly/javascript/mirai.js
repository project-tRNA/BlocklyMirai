...
; (function (root, factory) {
    ...
    function (Blockly) {
        ...
        Blockly.Mirai['contact_getid'] = function (block) {
            return ['.getId();', Blockly.Mirai.ORDER_ATOMIC];
        };
        // BlocklyMirai END
        // Java START
        ...
        Blockly.ConnectionChecker.prototype.doDragChecks = function (blockClick, blockWait, distance) {
            ...
            var requireTypes = blockClick.getSourceBlock().requireType;
            var returnTypes = blockWait.getSourceBlock().returnType;
            console.log("==================");
            console.log(requireTypes);
            console.log(returnTypes);
            // 检查类型
            if (typeof (requireTypes) != "undefined") {
                var flag_allow = false;
                if (typeof (returnTypes) != "undefined") {
                    for (var j = 0; j < requireTypes.length; j++) {
                        for (var k = 0; k < returnTypes.length; k++) {
                            if (requireTypes[j] == returnTypes[k]) {
                                flag_allow = true;
                                break;
                            }
                        }
