...
function(Blockly) {
	...
	Blockly.Constants.Logic.CONTROLS_IF_MUTATOR_MIXIN = {
		...
		reconnectChildBlocks_: function(a, b, c) {
			for (var d = 1; d <= this.elseifCount_; d++) Blockly.Mutator.reconnect(a[d], this, "IF" + d),
	...
	Blockly.Blocks.procedures_defnoreturn = {
		...
		domToMutation: function(a) {
			...
			this.argumentVarModels_ = [];
			for (var b = 0,
			c; c = a.childNodes[b]; b++) if ("arg" == c.nodeName.toLowerCase()) {
		...
		renameVarById: function(a, b) {
			...
			if ("" == c.type) {
				...
				for (var d = !1,
				e = 0; e < this.argumentVarModels_.length; e++) this.argumentVarModels_[e].getId() == a && (this.arguments_[e] = b.name, this.argumentVarModels_[e] = b, d = !0);
		...
		updateVarName: function(a) {
			for (var b = a.name,
			c = !1,
			d = 0; d < this.argumentVarModels_.length; d++) if (this.argumentVarModels_[d].getId() == a.getId()) {
	...
	Blockly.Blocks.procedures_mutatorarg = {
		...
		validator_: function(a) {
			...
			if (b.isInFlyout) return a; (b = c.getVariable(a, "")) && b.name != a && c.renameVariableById(b.getId(), a);
	...
	Blockly.Blocks.procedures_callnoreturn = {
		...
		setProcedureParameters_: function(a, b) {
			...
			if (b) if (a.join("\n") == this.arguments_.join("\n")) this.quarkIds_ = b;
				...
				this.argumentVarModels_ = [];
				for (e = 0; e < this.arguments_.length; e++) a = Blockly.Variables.getOrCreateVariablePackage(this.workspace, null, this.arguments_[e], ""),
				this.argumentVarModels_.push(a);
				this.updateShape_();
				if (this.quarkIds_ = b) for (e = 0; e < this.arguments_.length; e++) b = this.quarkIds_[e],
				b in this.quarkConnections_ && (f = this.quarkConnections_[b], Blockly.Mutator.reconnect(f, this, "ARG" + e) || delete this.quarkConnections_[b]); (this.rendered = c) && this.render()
			}
		},
		updateShape_: function() {
		...
		onchange: function(a) {
			if (this.workspace && !this.workspace.isFlyout && a.recordUndo) if (a.type == Blockly.Events.BLOCK_CREATE && -1 != a.ids.indexOf(this.id)) {
				...
				b = Blockly.Procedures.getDefinition(b, this.workspace); ! b || b.type == this.defType_ && JSON.stringify(b.getVars()) == JSON.stringify(this.arguments_) || (b = null);
				if (!b) {
					Blockly.Events.setGroup(a.group);
	...
	Blockly.Blocks['event_getsource'] = {
		init: function() {
			this.appendValueInput('content')
				.setCheck(null)
				.appendField("获取来源");
	...
	Blockly.Blocks['event_getsender'] = {
		init: function() {
			this.appendValueInput('content')
				.setCheck(null)
				.appendField("获取消息发送者");
	...
	Blockly.Constants.Text.QUOTE_IMAGE_MIXIN = {
		...
		quoteField_: function(a) {
			...
			e; e = c.fieldRow[d]; d++) if (a == e.name) {
	...
	Blockly.Constants.Text.TEXT_CHARAT_EXTENSION = function() {
		...
		this.setTooltip(function() {
			...
			c = Blockly.Msg.TEXT_CHARAT_TOOLTIP; ("FROM_START" == b || "FROM_END" == b) && (b = "FROM_START" == b ? Blockly.Msg.LISTS_INDEX_FROM_START_TOOLTIP: Blockly.Msg.LISTS_INDEX_FROM_END_TOOLTIP) && (c += "  " + b.replace("%1", a.workspace.options.oneBasedIndex ? "#1": "#0"));
			return c
		})
	};
